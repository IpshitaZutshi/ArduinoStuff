...
//Variables to change
int numLinTrials = 16; // First 10 trials are linear track
int randSeedNumber = 247; //input any random digits
int trialBlockNum = 81;//If trialBlocks is  1, after how many trials do you want linear trials to start again?
...
int equalDist = 1;//6 0 - probability of ports is skewed towards later ports
                  // 1 - equal probability of ports
                  // -1 - oprobability according to toneGain 4
                  // 2,3,4,5,6,7 - port number x+2 is the only one rewarded.
int numPorts = 6;// 3 or 6                  
int trialBlocks = 1; // Flag for whether to run linear track and tone trials in blocks
...
int cutoffFreq = 4000; // Frequency after which the tone stops playing in probe trial
int probeTrial = 0; // Flag for probe trial
int probeTrialType = 0; // Flag for probe trial type
                        // 0 - stop tone
                        // 1 - continue tone
...
int toneGainOptions[10] = {1,2,3,4,4,5,5,6,6,6};
int toneGainOptions2[4] = {2,4,6,6};
int toneGainOptions3[5] = {3,4,5,2,1}; //1,2,3,4 change according to which ports should be rewarded
int toneGainOptions4[10] = {1,2,3,3,4,4,5,5,6,6}; // for probe trials
const int arrLen = sizeof(toneGainOptions3[4]) / sizeof(toneGainOptions3[0]);

int probeTrialOptions[10] = {0,1,0,0,0,0,0,0,1,1}; // Change according to probability of probe trials

int gainFactor[6] = {55,130,218,306,390,450};
int maxDim = 450;
int wrongTrial = 0;
int cur_gain_num;
int probe_trial_num;
  ...
  if (linearTrackTrials ==1){  
    ...
    if (prevTrialCount<countTrials && returnRun == 0){
     ...
     if (numPorts==6){
      ...
      probe_trial_num = random(0,10); // Select 0 or 1 for probe trial flag 
     } else if (numPorts==3){
      cur_gain_num = random(0,4);  
      probe_trial_num = random(0,4);
     }
      
     if (equalDist==1){ 
      if (numPorts==6){
          // cur_gain = random(0,6);
          cur_gain = toneGainOptions4[cur_gain_num]-1; // for probe trials only
          if ((cur_gain==2) || (cur_gain==3) || (cur_gain==4)|| (cur_gain==5)){
            probeTrial = probeTrialOptions[probe_trial_num];
          }
          else{
            probeTrial = 0;
          }
        ...
        if ((cur_gain==2) || (cur_gain==3) || (cur_gain==4)){
            probeTrial = probeTrialOptions[probe_trial_num];
          }
          else{
            probeTrial = 0;
        ...
        cur_gain_num = random(0,5);
        cur_gain = toneGainOptions3[cur_gain_num]-1;
       // probeTrial = probeTrialOptions2[probe_trial_num];
     } else {      
    ...
    if (returnRun==0){
      ...
      // Probe trials: stop or continue same tone after crossing the cutoff frequency
      else{
        if ((map_pos<1) && (cur_pos<1)){
          if (thisPitch < cutoffFreq){
            analogWrite(positionAudioPin, map_pos2); //Send analog tone being played to intan
            // play the pitch:
            toneAC(thisPitch,10);
          }
            ...
            if (probeTrialType == 0){
              ...
              toneAC(cutoffFreq,10); // Continue playing tone
            }
          }
        }
        // Beyond the target port
        else{ 
          if (probeTrialType == 1){
            toneAC(cutoffFreq,10); // Continue playing tone
          }
