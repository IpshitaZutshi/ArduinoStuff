...
int phase = 3; //if phase = 1 (training), one door open + extended cue 
               //if phase = 2 (training), both doors open + extended cue
...
int optPhase = 0;   //0 = no stim
                    //1 = during cue only
                    //2 = during delay only
                    //3 = cue + delay (the 5 seconds)
                    //4 = delay + going to right/left nosepoke 

int blocksSize = 11;//n+1                    
...
const int numRandTrials = 5;// variable to make sure that the left and right trials are equally presented
...
int doorBaseDelay = 2000;      //DEFAULT IS 2000        //cueDelay and doorbasedelay are no longer additive       
int varyCue = 1;// if 0, cueDelay is constant, if 1 cueDelay changes;
...
const int IRbasepin = 43;    //intan pin 9
...
int pastTrials[numRandTrials];
int newTrials[numRandTrials];
float trialbias = -1;
...
void setup() {
  ...
  pinMode(IRbasepin, OUTPUT); digitalWrite(IRbasepin, LOW);
...
void loop(){
  ...
  if(state == 0) {                                             //state = 0 means home base
      ...
      if (biasCorrect==0){ // Don't correct for mouse's bias, but make sure that the arduino doesn't generate way more trials towards a certain side
         if (countTrials<numRandTrials){  
            randNumber = random(0,2);           
              ...
              randNumber = 1;//randNumberProb(trialbias); //make it so that randNumber is 0 or 1 with the probability of past trials;                     
         }
         for (int ii = 0; ii<(numRandTrials-1); ii++){
           newTrials[ii+1] = pastTrials[ii]; 
         }
         memcpy(pastTrials,newTrials, sizeof(newTrials));
         newTrials[0] = randNumber;
          ...
          memcpy(pastchoices,newchoices, sizeof(newchoices));
      }
    ...
    if(randNumber == 0) {
      if(digitalRead(IRleft) == HIGH) {
        if(optPhase == 4) {                   //opto turns off for delay + right/left nosepoke phase
      ...
      //  digitalWrite(IRrightpin, HIGH); delay(10); digitalWrite(IRrightpin, LOW);
        ...
        doorLeftState = false;          
        tone(speakerLeft, 3000); tone(speakerRight, 3000);                            //if they get it wrong, both speakers play a sound
        delay(speakerDelay);
        noTone(speakerLeft); noTone(speakerRight); 
        state = 2;
        newchoices[0] = 1;
      ...
      //  digitalWrite(IRrightpin, HIGH); delay(10); digitalWrite(IRrightpin, LOW);
        digitalWrite(solRight, HIGH); digitalWrite(solPin, HIGH); delay(solenoidDelay); digitalWrite(solRight, LOW); digitalWrite(solPin, LOW);
        digitalWrite(ledRight, LOW); 
        digitalWrite(rightCue, LOW);       
        noTone(speakerRight);        
    ...
    if(digitalRead(IRbase) == HIGH) {
      //if (wrongTrial == 0){
        digitalWrite(solBase, HIGH); digitalWrite(solPin, HIGH); 
        delay(solenoidDelay); 
        digitalWrite(solBase, LOW);digitalWrite(solPin, LOW);
      //}
      if(doorLeftState) {                                             
        servo1.write(closeDoor);
      }
  ...
  digitalWrite(IRbasepin,digitalRead(IRbase));
  digitalWrite(IRrightpin,digitalRead(IRright));
  digitalWrite(IRleftpin,digitalRead(IRleft));
