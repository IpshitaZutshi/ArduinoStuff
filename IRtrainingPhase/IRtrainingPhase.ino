...
int phase = 2; //1 mean the water will come out of solenoid as long as IR bream is broken. phase 2 will mean the water will only come out for a specific amount of time and will eventually stop. Then there will be a cooldown
...
void loop() {
  ...
  if(phase == 2) {
    if(digitalRead(IRbase) == HIGH) {
    digitalWrite(solBase, HIGH); delay(50); digitalWrite(solBase, LOW);              //water will be given for 0.5 seconds and then stop. Cooldown is 6 seconds
    delay(1000);
    }
    if(digitalRead(IRleft) == HIGH) {
    ...
    if(digitalRead(IRright) == HIGH) {
    digitalWrite(solRight, HIGH); delay(50); digitalWrite(solRight, LOW);
    delay(1000);
    }
  }
